...
fn sample_vegetation_with_biome_blend(
    ...
    // Fast path: far from any biome boundary (server blend factor B < 0.01) → a
    // single palette, no blur. Both paths now carry a real distance-based blend
    // factor — Azgaar from its triangulation, the Ymir path from biome.u8 — so
    // this cheap path handles biome interiors and the multi-sample blur below runs
    // only at boundaries (B >= 0.01). (Replaces the always-on Ymir blur from 8d28819.)
